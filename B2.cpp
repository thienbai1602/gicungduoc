...
struct bigint {
    ...
    bigint operator+(const bigint &v) const {
        if (sign == v.sign) {
            bigint res = v;
            for(int i=0, carry=0; i<max(sz(res.a), sz(a)) || carry; i++)
            {
...
struct bigint
{
    ...
    {
        bigint res = v;
        for(int i=0, carry=0; i<max(sz(a), sz(res.a)) || carry; i++)
        {
            if (sz(res.a) == i) res.a.pb(0);
            res.a[i] += (carry + (i < sz(a) ? a[i] : 0));
            carry = res.a[i] >= base;
            if (carry)
            {
    ...
    friend ostream& operator << (ostream& stream, const bigint& v)
    {
        stream << (sz(v.a) == 0 ? 0 : v.a.back());
        for(int i=sz(v.a)-2; i>=0; i--)
        {
            stream << setw(base_digit) << setfill('0') << v.a[i];
        }
        return stream;
    }
};
